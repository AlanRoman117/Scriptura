...
| 1.3.2 | Meaningful Sequence | ✅ | review of DOM order (not automated) | DOM order matches reading order in every state: the passage bar, search, the chapter, then notes; the verse actions follow their verse in the DOM even when docked at the bottom of a phone screen; the board's panel follows the cards. |
...
| 2.4.3 | Focus Order | ✅ | `keyboard.spec.ts` (Escape returns focus to the opening chip for every panel; verse actions take focus when opened from the number, are one Tab stop, and return focus to the number; the dialog returns focus; Escape closes only the newest surface); `reading.spec.ts` (skip link); `chrome.spec.ts` (each maximize button is the next Tab stop after its bar's last control, where it is drawn) | `useReturnFocus` and the dismiss stack in `lib/focus.ts`; roving tabindex in the verse actions; skip links first in the document; the maximize buttons are in their bars' DOM order, not positioned into a corner. |
...
| 3.1.1 | Language of Page | ✅ | `index.html` `lang="en"`; axe `html-has-lang`, `html-lang-valid` in every state | |
...
| 4.1.2 | Name, Role, Value | ✅ | axe `button-name`, `aria-allowed-attr`, `aria-allowed-role`, `aria-progressbar-name` in every state including the board; `editor.spec.ts` (the toolbar is one Tab stop with arrow keys); `canvas.spec.ts` (cards are named focusable groups) | Named glyph buttons, a named progressbar, disclosures with `aria-expanded`/`aria-controls`, the storage notice a named landmark, a real toolbar, the verse actions a named group with roving focus, board cards named groups that take focus, and the board's panels named groups. |
...
| 1.4.10 | Reflow | ✅ | `tests/reader-touch/gates.spec.ts` → "reflow at 320 CSS px": no horizontal overflow of the page, the panes, the sheet or the suggestions in nine phone states, with a planted-overflow self-check; `tests/reader-touch/compare.spec.ts` | The layout switches to one pane with a notes sheet below 850px; the reading bar wraps below 26rem; a comparison is a list of verses on a narrow pane. The canvas is two-dimensional content and is exempt. |
