...
| 1.4.11 | Non-text Contrast | 🔧 W2 | `contrast.test.ts` (`--edge`, `--focus`, `--hl-*-strong` ≥ 3:1 in every theme) | Done: every control border, the divider grip, the sheet handle, the swatch ring and card colour borders use `--edge` or a strong hue. Remaining: the highlight left rule (W2). |
...
| 2.4.11 | Focus Not Obscured (Minimum) | 🔧 T1 | `chrome.spec.ts` (the sticky title parks exactly under the measured bar and search heights) | Done: `--bar-h` and `--search-h` are measured with a `ResizeObserver` and drive the sticky offsets and `.pane`'s scroll padding. Remaining: the Bible pane goes `inert` under a full notes sheet (T1). |
...
| 2.5.8 | Target Size (Minimum) | 🔧 T2, T4, T7 | `tests/reader/targets.spec.ts` (every visible target ≥ 44 × 44 in seven states) | Superseded by 2.5.5, which is met in the same states. Remaining states: verse actions (T2), canvas (T4), comparison (T7). |
...
| 2.5.5 | Target Size (Enhanced) | 🔧 T2, T4, T7 | `tests/reader/targets.spec.ts`: reading, note open, preview, Marks, Translations, Settings, results — every visible pointer target measures ≥ 44 × 44 CSS px, allowlist of inline exceptions with a reason each | `--control-h: 44px` applied as `min-height` (and `min-width` for icon-only and short-label buttons) to every control class in one place in the stylesheet. Exceptions claimed: the verse number (inline in the text; the whole verse is the equivalent target) and text links inside prose. Remaining states: verse actions (T2), canvas (T4), comparison (T7). |
