...
| 2.5.8 | Target Size (Minimum) | 🔧 T4 | as 2.5.5 | Superseded by 2.5.5, met in the same states. |
...
| 2.5.5 | Target Size (Enhanced) | 🔧 T4 | `tests/reader/targets.spec.ts` (twelve desktop states) and `tests/reader-touch/gates.spec.ts` (nine phone states), one measurer in `tests/helpers/targets.ts` | `--control-h: 44px` on every control class; swatches are 44px buttons around a 26px disc. Exceptions claimed: the verse number (inline in the text; the whole verse is the equivalent target) and text links inside prose. Remaining: the board (T4). |
