...
6. Single-pointer alternatives (2.5.7): each card's footer gains a "Move/Size" button (`data-testid="card-adjust-{id}"`) opening a small popover with four arrow buttons (move 20 px), four ± buttons (width/height ± 20 px) and Close, all 44 px, dismissed through the stack; `.canvas__zoom` gains four pan buttons (`data-testid="pan-left|right|up|down"`, 80 px each).
7. The five colour dots are replaced by one 44 px "Colour" button (`data-testid="card-colour-{id}"`, `aria-label="Colour: {label or none}"`) that opens the same swatch row component as brief 12; the card keeps `data-color` and gains a visually-hidden colour name in its title. `CARD_W = 260`, `CARD_H = 180`; `.card__resize` is 24 px visible with a 44 px hit box; `.card__actions` buttons are ≥ 44 px (the footer becomes two rows on narrow cards).
