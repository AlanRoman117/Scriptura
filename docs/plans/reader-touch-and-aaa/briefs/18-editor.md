...
3. Tools remember the textarea selection: on `focusin` of the toolbar, read and store `selectionStart/End`; `apply()` uses the stored range if the textarea is not focused. `onMouseDown={hold}` is removed (pointer users get the same stored-range behaviour).
