...
9. **Delete.** The Delete button is announced as opening a dialog. Press it: an alert dialog is announced with its question ("Delete the note “…”?") and then what happens ("It will be deleted from this device. You cannot undo this. …"); focus is on Cancel. Escape: focus is back on Delete and the note is kept. Press Delete again, Tab to "Delete note", Enter: focus lands on the note picker, which says which note is open now, and "Deleted the note “…”" is announced after it, not over it. In the Marks panel, remove a mark the same way: focus moves on to the next mark, and Escape in the dialog leaves the panel open.
