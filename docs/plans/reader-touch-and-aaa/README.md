...
| T4 | Canvas: touch, pinch, keyboard, connections list, undo | `briefs/14-canvas.md` | done (7909910, 441fc0b, 363afda, b9f6573) |
...
| W2 | Highlight markers and non-colour meaning | `briefs/22-colour-meaning.md` | done (8ad97e8; board cards in 363afda) |
...
- **`lang` sat on containers that also held controls**, so verse numbers, verse actions, comparison quote buttons and board card names were spoken in the open Bible's voice. It went unnoticed while the interface was English-only and most Bibles were read in English. Fixed with the interface languages (`docs/plans/reader-i18n/`), and `tests/reader/language-of-parts.spec.ts` now checks both sides of the boundary.
...
- **"Start one" on the board screen did nothing when pressed** with a mouse or a finger; only the keyboard could start a first board. The no-board message shared its class with the hint drawn over an empty board, whose `pointer-events: none` lets a drag pan through it. No test pressed that button. Found in a manual review after the i18n merge and fixed on `reader/confirm-dialog`; `confirm.spec.ts` now presses it with a hit-tested click.
- **The two-press delete was too quiet.** The same review found that a button turning into "Sure?" where it stood was easy to miss. Deleting now asks in a modal alert dialog (`ConfirmDialog`) that names what goes and opens on Cancel, on `reader/confirm-dialog`.
