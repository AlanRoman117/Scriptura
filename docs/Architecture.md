...
|---|---|---|
| Display preferences | `src/lib/prefs.ts`, inline script in `index.html` | Style (Classic, High contrast, Sepia, Vellum, Emerald, Slate, Nocturne, Ember) and appearance (light, dark, or the device's), text size 100–200 %, spacing presets that meet 1.4.8, column width ≤ 70 ch, reduced motion, non-colour markers. Device-scoped in `localStorage`; applied before first paint; `system` is an *absent* attribute so `prefers-*` stay the defaults. |
| Design tokens | `src/styles.css`: `@tokens <style>`, `@derived`, `@style` blocks | A palette per style in `light-dark()` pairs; derived fills written as `color-mix()` formulas over it; and the style tier — radii, edge width, elevation, the type scale, the reading face, how titles and verse numbers are set. Component rules use tokens only. `--edge` is for control boundaries and `--rule` for decoration only. `tests/unit/contrast.test.ts` evaluates the formulas and checks 944 pairs across 8 styles × 2 polarities: 7:1 for text, 3:1 for edges, focus rings and highlight rules. |
...
**What the reader can do without a mouse, and without dragging.** Every function is keyboard-operable (2.1.3) and every drag has a single-pointer alternative (2.5.7): the pane divider is a bar holding Narrower/Wider buttons, the notes sheet's grip is a button, a board card has Move/Size and Colour panels of plain buttons, and the board view has pan buttons. The pane beside the Bible holds the notes or the canvas, chosen by two tabs, and either can fill the window, so a reader building a diagram swaps to the note and back without leaving the text; search results go onto the board as easily as into a note. On a phone the notes sheet drags and snaps, holds the same two tabs, focusing the note makes it full, the text it covers is `inert`, the verse actions dock above it, and a comparison reads as a list of verses instead of a sideways-scrolling table.
...
|---|---|---|
| `reader` | Desktop Chrome | axe-core at the WCAG 2.0–2.2 A/AA/AAA tags in fifteen states and both colour schemes, in five states for each other style in both polarities, and in eleven states in each of the other three interface languages, over the whole document, with a self-check that the two AAA rules and the page-level rules (`bypass`, `document-title`, `html-has-lang`, `region`) really ran; 44 px targets in thirteen states; keyboard traversal, focus visibility and focus return; reduced motion; a real service-worker update served from a private origin |
...
Markers off the caret's line are clipped to a pixel, not removed, so a screen reader still reads each line as its Markdown (`## Covenant`, `**promise**`) and learns that a line is a heading or a word bold; hiding them with `display: none` took that structure out of the accessibility tree (1.3.1). The focus ring is drawn inside the note and its title, so both are padded to keep it clear of the caret. Each line takes its direction from its own text (`dir="auto"`), so a Hebrew or Arabic line runs right to left beside English or Japanese; direction is not tied to the interface language, because a note's language is its writer's. **Plain text** under *Note editor* in Settings brings back the Markdown source in a textarea, and only there is **Preview** offered: the note rendered as a document, with real headings. In Live the note is already drawn, so a second view would only be a mode to leave.
...
The plan, glossary and translation review status are in [`docs/plans/reader-i18n/`](plans/reader-i18n/README.md). The Spanish, French and Japanese copy are drafts until a native speaker of each has reviewed them.
...
| v4.2 | Reader interface in English, Spanish, French, Japanese, Brazilian Portuguese and Chinese in both scripts ✅ — native review of the non-English copy ⏳ planned ([plan](plans/reader-i18n/README.md)) |
