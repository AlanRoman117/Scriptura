...
const helpSections = (book: BookExample): HelpSection[] => [
  ...
  },
  {
    ...
    blocks: [
      {
        p: 'ノートは Markdown で書くテキストです。書式は書いているそばから反映され、編集中の行では記号が表示されます。Markdown をそのまま見たいときは、設定の「ノートの編集」で**プレーンテキスト**を選んでください。ノートの上のツールバーで、見出し、太字、斜体、リスト、引用を付けられます。同じツールをもう一度押すと書式が外れます。**プレビュー**で、仕上がりを確認できます。',
      },
      {
  ...
  },
  {
    ...
    blocks: [
      {
        p: 'ボードは、節やノートをカードとして並べ、矢印でつなげる場所です。節の横の**ボードへ**を押すと、その節が今のボードに加わります。カードには本文の写しではなく参照箇所が入っているので、読んでいる訳で表示されます。ボードの名前は、ボードのメニューの横の欄で付けられます。',
      },
      {
...
export const jaJP = {
  ...
  previewBuild: {
    label: 'プレビュー版',
    intro: '**プレビュー版です。**翻訳を確認していただく方のための早期版です。',
    report: 'GitHubで翻訳の修正を報告する（新しいタブで開きます）',
    reportShort: '修正を報告する',
    newTab: '（GitHub、新しいタブで開きます）',
    hide: '非表示',
    heading: 'このプレビュー版について',
    about:
      'この版は、スペイン語・フランス語・日本語・中国語・ポルトガル語の画面表示を確認していただく方のためのものです。ここでの操作内容はどこにも送信されません。ノートとマークはこのブラウザーの中に残ります。誤りやわかりにくい言葉を見つけたら、下のリンクから報告してください。GitHubの短いフォームが開きます（無料のアカウントが必要です）。',
