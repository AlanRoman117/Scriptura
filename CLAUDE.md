...
- **`ingest.py`** — Downloads open-license source data and normalizes it into the canonical schema. Implemented with two parsers behind a `TRANSLATIONS` registry:
  ...
  - Automated and clean today: `rv1909`, `kjv`, `web`, `lsg1910`, `vbl`. Automated but eBible IDs need confirmation: `asv`, `ylt`. Registered but require a dedicated source/parser (script skips with a note): `bsb`, `martin1744`, `ostervald`, `bungo`.
...
Ingestion status (see `scripts/ingest.py`): `rv1909`, `kjv`, `web`, `lsg1910`, `vbl` ingest cleanly; `asv`, `ylt` are automated pending eBible ID confirmation; `bsb`, `martin1744`, `ostervald`, `bungo` need a dedicated source/parser.