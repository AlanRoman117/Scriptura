...
The REST API runs (`npm run dev:api`), serves every translation in every language, and its responses are byte-identical to the static build. The reader PWA (`apps/reader`) is through **Stage 4 of 5**: offline reading, notes, colour collections, export, offline search and linking, a downloadable translation library, and side-by-side comparison. 126 jest and 106 Playwright tests pass. CI exists. The remaining gaps are the canvas/WebMCP stage, GraphQL, and the AWS deployment workflow — all still design-only (see Docs → Specified but not built).
...
Jest with ts-jest. Config in root `jest.config.ts`. **126 jest tests, plus 106 Playwright** (42 contract, 62 reader, 2 reader-dev).
