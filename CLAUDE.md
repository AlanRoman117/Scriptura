...
Jest with ts-jest. Config in root `jest.config.ts`. **147 jest tests, plus 143 Playwright** (42 contract, 99 reader, 2 reader-dev).
...
- **`apps/reader`** — the local-first reading and note-taking PWA. Vite + React 19 + TypeScript. `npm run dev:reader` / `npm run build:reader`.
  ...
  - **`EditorToolbar` rides in with focus and leaves with it.** "Collapsed by default" cannot mean absent: nobody discovers that `##` makes a heading by staring at a textarea. ⚠️ Every control `preventDefault`s `mousedown` — a button that takes focus collapses the selection it was about to format.
  ...
  - **A board embeds in a note as a fence** (` ```scriptura-board `) holding its id, and renders as a diagram, the role a mermaid fence plays. On **export the fence is replaced with the board's Markdown** (`inlineBoardEmbeds`): in the app it draws, but a `.md` opened anywhere else would show a code block containing a UUID. ⚠️ The embed's SVG `marker` id is scoped to the board — two embeds in one note would otherwise collide.
