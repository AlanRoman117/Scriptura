...
- **`apps/reader`** — the local-first reading and note-taking PWA. Vite + React 19 + TypeScript. `npm run dev:reader` / `npm run build:reader`.
  ...
  - ⚠️ **The panel reopens on typing, not only on focus.** Enter closes it to clear the way for the passage it jumped to, but the input keeps DOM focus — so without this a reader typing a second reference gets no panel and no feedback at all, and no `focus` event ever fires to bring it back.
  - ⚠️ **In tests, wait for `search-jump` before pressing Enter.** The reference resolves in an effect, so firing Enter straight after `fill` races it, and a lost Enter is indistinguishable from a reference that failed to parse. Ubuntu won that race; macOS lost it. And never assert `John 1` in a jump test — the reader opens on it, so the assertion passes without anything having happened, which is how the race stayed invisible.
