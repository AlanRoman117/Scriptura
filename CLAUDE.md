...
- **`ingest.py`** — Downloads open-license source data and normalizes it into the canonical schema. Implemented with two parsers behind a `TRANSLATIONS` registry:
  ...
  - Automated and clean today (9): `rv1909`, `kjv`, `web`, `lsg1910`, `vbl`, `asv`, `ylt`, `bsb`, `ostervald`. All eBible IDs are confirmed against <https://ebible.org/Scriptures/translations.csv> (the authoritative index — check it there before guessing an id). Still registered as `manual`, needing a dedicated source/parser (script skips with a note): `martin1744`, `bungo`.
...
Ingestion status (see `scripts/ingest.py`): 9 of 11 ingest cleanly and pass `validate.py --strict` with zero warnings — `rv1909`, `kjv`, `web`, `lsg1910`, `vbl`, `asv`, `ylt`, `bsb`, `ostervald`. Only `martin1744` (Bible Martin 1744) and `bungo` (文語訳聖書) still need a dedicated source/parser; neither is on eBible.org, so `data/martin1744/books/` and `data/bungo/books/` are empty and `validate.py` currently reports 2 errors.