...
- **`apps/reader`** — the local-first reading and note-taking PWA. Vite + React 19 + TypeScript. `npm run dev:reader` / `npm run build:reader`.
  ...
  - **`EditorToolbar` is always shown above a note being written** — only preview, which has nothing to format, drops it. It used to ride in with focus and leave with it; the owner reversed that after the AAA work, because tools that appear and vanish as focus moves are hidden from anyone who has not yet clicked into the note and hard to trust, and nobody discovers that `##` makes a heading by staring at a textarea. ⚠️ Do not bring back a focus-dependent toolbar; `tests/reader/editor.spec.ts` asserts the tools are visible before the note is touched and after focus leaves it. It sat *below* the surface at first, which in a long note puts the heading button a scroll away from the text it applies to. The toolbar is one Tab stop with arrow keys, reached by Tab from the title or Shift+Tab from the note. One `mousedown` guard stays, on the toolbar element, so a pointer press on a tool leaves focus and the selection in the note; the keyboard path needs no stored selection — a textarea keeps its selection range while blurred.
